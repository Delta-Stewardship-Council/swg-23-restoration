...
sample <- st_read(file.path(path_to_data, "Prop1_1605/Prop1_1605.shp"))
sample %>%
  head() %>%
  kbl()
```
...
Based on the table above, we want the Petersen Ranch project. In this case, I had to inspect the title from the original proposal and match it manually with this table. 
...
This section needs to be completed. Here's what's left to do: take the converted list object of multiple tibbles, and convert it to a few aggregated tibbles. Then these should be saved to a few different csv files. Alternatively, save to a sqlite object.

---

Due to the complexity of the JSON object, below I create custom functions to handle the conversion of this object into a properly structured tibble. The first chunk below are helper functions, and the second chunk actually creates a single tibble object.
...
Now, convert this list of tibbles to a single tibble.

:::{.callout-note}
The reason this processing of the JSON file is split into two chunks is that the first chunk generically converts list children into tibbles, while the second chunk is bespoke to this output only. In other words, I haven't tested this code on other projects, so I can't assume this works universally.
:::

```{r}
#| code-summary: Create a single tibble from this complicated list.
p2 <- fn_convert1(p)
...
```{r}
p2$sites[[1]]$other
p2$sites[[1]]$activities$activity
p2$sites[[1]]$site
```
